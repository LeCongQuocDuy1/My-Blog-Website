...
const Select = ({label, options = [], register, errors, id, validate, active }) => {
  return (
    <div className='flex flex-col gap-2 mb-[20px]'>
      ...
      <select id={id} {...register(id, validate)} className='p-[10px] cursor-pointer text-orange-600'>
        ...
        {options.map(option => (
          <option selected={active === option.code ? true : false} key={option.code} value={option.code}>{option.value}</option>
        ))}
      </select>
